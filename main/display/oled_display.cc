...
void OledDisplay::DrawFillRectangle(void *ctx, int32_t x0, int32_t y0, int32_t w, int32_t h, uint16_t color)
{
    DrawFillRectangleRound(ctx, x0, y0, w, h, 0, color);
}

void OledDisplay::DrawFillRectangleRound(void *ctx, int32_t x0, int32_t y0, int32_t w, int32_t h, int32_t radius, uint16_t color)
{
    if (color == 0)
        rect_dsc_.bg_color = lv_color_white();
    else if (color == 1)
        rect_dsc_.bg_color = lv_color_black();
    rect_dsc_.radius = radius;
    lv_area_t area;
    area.x1 = x0;
    area.y1 = y0;
    area.x2 = x0 + w - 1;
    area.y2 = y0 + h - 1;
    lv_draw_rect((lv_layer_t *)ctx, &rect_dsc_, &area);
}

void OledDisplay::DrawFillTriangle(void *ctx, int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2, uint16_t color)
{
    if (color == 0)
        triangle_dsc_.color = lv_color_white();
    else if (color == 1)
        triangle_dsc_.color = lv_color_black();

    triangle_dsc_.p[0].x = x0;
    triangle_dsc_.p[0].y = y0;
    triangle_dsc_.p[1].x = x1;
    triangle_dsc_.p[1].y = y1;
    triangle_dsc_.p[2].x = x2;
    triangle_dsc_.p[2].y = y2;
...
void OledDisplay::SetupUI_128x64()
{
    ...
    lv_obj_set_size(eye_container_, 128, 48);
    ...
    // lv_obj_add_event_cb(eye_container_, drawEyesEventCallbackk, LV_EVENT_DRAW_TASK_ADDED, NULL);
    

    //load default screen
    lv_screen_load(screen_main_);
    // lv_obj_add_flag(eye_container_, LV_OBJ_FLAG_SEND_DRAW_TASK_EVENTS);
}
...
void OledDisplay::SetHide(bool hide)
{
    DisplayLockGuard lock(this);
    ...
    //     lv_obj_clear_flag(eye_container_, LV_OBJ_FLAG_HIDDEN);
    //     lv_screen_load(screen_aux_);
    //     draw_eye_timer_ = lv_timer_create(eyeTimerCallback, 50, eye_container_);
    // }
    // else
    // {
